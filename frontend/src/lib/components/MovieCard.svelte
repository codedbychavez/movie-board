...
<div class="movie-card max-w-2xs min-h-32 border border-gray-200 rounded shadow-sm p-3 shrink-0 bg-gray-50">
  ...
  <div class="mt-4 flex flex-wrap gap-2 justify-between">
    ...
    <select bind:value={selectedStatus} onchange="{() => updateMovieStatus(movie)}" class="bg-purple-500 rounded-full px-2 py-1 text-white">
    ...
    <!-- <button onclick="{() => updateMovieStatus(movie)}" class="bg-teal-500 text-green-50 px-3 py-1 rounded-full cursor-pointer">
      {movie.status}
    </button> -->
